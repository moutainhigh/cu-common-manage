...
public class CreateXmlTools {
	private static Logger logger = Logger.getLogger(CreateXmlTools.class);

	/**
	 * 创建上报xml文件，并使用xml校验文件内容，成功向redis添加一个ack上报任务
	 * 
	 * @param type 文件类型参见:com.aotain.common.utils.ismi.ConstantParams.DT_*
	 * @param idcId 经营者ID
	...
	public static Result validateAndCreateXml(int type, String idcId, Object bean, Long topTaskId,Integer... recordCounts) {
		...
		try {
			...
			logger.info("toXmlString spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logger.info("xsd validate spend time:" + (endTime - startTime) + " ms");
			if (!StringUtils.isBlank(error)) {
		...
		logger.info("taskId:" + taskId + ",fileName:" + fileName);
		try {
			long startTime = System.currentTimeMillis();
			data = ZipUtils.zip(xmlStr, fileName + ".xml", "UTF-8");
			long endTime = System.currentTimeMillis();
			logger.info("zip data spend time:" + (endTime - startTime) + " ms");
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logger.info("create upload data spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logger.info("create upload xml string spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
		...
		try {
			...
			outStream.flush();
			long endTime = System.currentTimeMillis();
			logger.info("create xml tmp file spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
		...
		long endTime = System.currentTimeMillis();
		logger.info("add data to redis spend time:" + (endTime - startTime) + " ms");
		// 完成后修改文件名
		try {
	...
	public static Result validateAndCreateXmlToCacheDir(int type, String idcId, Object bean,
		...
		Result result = new Result();
		// 入参格式校验
		logger.info("[validateAndCreateXmlToCacheDir]validateParams start");
		...
		String identify = "type=" + type + ",data-class=" + bean.getClass().getName();
		logger.info("[validateAndCreateXmlToCacheDir]identify:" + identify);
		String xmlStr = null;
		// 1. 在内存中创建xml文件
		try {
			long startTime = System.currentTimeMillis();
			xmlStr = XmlUtils.toXmlString(bean);
			long endTime = System.currentTimeMillis();
			logger.info("[validateAndCreateXmlToCacheDir]toXmlString spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
			result.setError("create xml exception : " + identify);
			logger.error(result.getError(), e);
			return result;
		...
		try {
			...
			error = UploadFileXsdValidator.getInstance().doValidate(type, new ByteArrayInputStream(xmlStr.getBytes("UTF-8")));
			long endTime = System.currentTimeMillis();
			logger.info("[validateAndCreateXmlToCacheDir]xsd validate spend time:" + (endTime - startTime) + " ms");
			if (!StringUtils.isBlank(error)) {
				result.setError("xml-file xsd validate fail - " + error + " : " + identify);
				logger.error(result.getError());
				return result;
		...
		result.setTaskId(taskId);
		String fileName = createFileName(taskId);
		logger.info("[validateAndCreateXmlToCacheDir]taskId:" + taskId + ",fileName:" + fileName);
		try {
			data = ZipUtils.zip(xmlStr, fileName + ".xml", "UTF-8");
			if (data == null || data.length == 0) {
				result.setError("xml-file data is empty after zip compress : " + identify);
		...
		try {
			...
			upload.setCommandVersion(wsVersion);
			long endTime = System.currentTimeMillis();
			logger.info("[validateAndCreateXmlToCacheDir]create upload data spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
			result.setError("xml-zip-file  encrypt exception : " + identify);
			logger.error(result.getError(), e);
			return result;
		...
		try {
			...
			xmlStr = XmlUtils.toXmlString(upload);
			long endTime = System.currentTimeMillis();
			logger.info("[validateAndCreateXmlToCacheDir]create upload xml string spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
			result.setError("create result-xml exception : " + identify);
			logger.error(result.getError(), e);
			return result;
		...
		try {
			...
			long endTime = System.currentTimeMillis();
			logger.info("[validateAndCreateXmlToCacheDir]create xml tmp file spend time:" + (endTime - startTime) + " ms");
		} catch (Exception e) {
	...
	public static class Result {
		...
		public String toString() {
			return "Result [error=" + error + ", xmlPath=" + xmlPath + ", taskId=" + taskId + "]";
		}
