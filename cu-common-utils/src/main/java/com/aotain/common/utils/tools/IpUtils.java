...
public class IpUtils {
	...
	public static String ipv6InnerV4ToStandard(String ipv6) {
		...
		if (m.find()) {
			...
			// 若ipv6Pre为：1:1:1:1:1:1的格式时，前缀不用修改
			if(!ipv6Pre.contains("::")){
				return ipv6Pre + ":" + ipv4ToV6Suffix(ipv4);
			} else {
	...
	public static void main(String[] args) {
		System.out.println(isIpv6("2002:9D36:1:2:0:5EFE::10.10.10.10"));
		// ip 格式判断测试
		List<String> ipList = FileUtils.readFileByLine(new
		File("config/ip-store2.txt")); 
		testIp(ipList); 
		testIpv4(ipList);
	...
	public static String buildIpv6Sql(String fieldName, String ip) {
		...
		if (IpUtils.isIpv4(ip)) {
			String ip1 = ip;
			String ip2 = IpUtils.ipv4ToInnerV4(ip);
			ipList.add(ip1);
			ipList.add(ip2);
			return " and ( " + buildIpSql(fieldName, ipList) + " ) ";
		} else if (IpUtils.isIpv6(ip)) {
			byte[] ipv6Datas = IpUtils.ipv6ToBytes(ip);
			String ipv6Std = IpUtils.parseIpv6(ipv6Datas, false);
			String ipv6Compress = IpUtils.parseIpv6Compress(ipv6Datas, false);
			String ipv6Inner = IpUtils.parseIpv6InnerV4(ipv6Datas, false);
			ipList.add(ipv6Std);
			ipList.add(ipv6Std.toUpperCase());
			if (!ipv6Std.equals(ipv6Compress)) {
				ipList.add(ipv6Compress);
