...
//			mapper.deletePolicyRule(domain);
	...
	public boolean addCustomLogic(BaseVo policy) {
		...
		boolean ispPolicyBatchOperationFlag = false;
		if ( policy instanceof ISPMessagePolicy) {
			ISPMessagePolicy ispMessagePolicy = (ISPMessagePolicy)policy;
			int batchOperationFlag = ispMessagePolicy.getBatchOperationFlag();
			if (batchOperationFlag == 1) {
				ispPolicyBatchOperationFlag = true;
			}
		}
		if (!ispPolicyBatchOperationFlag) {
