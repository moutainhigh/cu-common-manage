...
import com.aotain.common.utils.util.MonitorContentToFileUtil;
...
public class ExceptionMonitorTask {
    ...
    @Autowired
            @Qualifier("baseRedisServiceImpl")
...
//            writeServerStatusFile();
            ...
            exceptionCollector.addEvent(ModuleConstant.MODULE_TASK_MONITOR,e);
        }
    ...
    private boolean updateThreadStatus(String module,String type){
        boolean result = true;
        try{
            String field = String.format(ExceptionCollector.THREAD_MONITOR_KEY,module,type);
            String value = baseRedisService.getHash(ExceptionCollector.THREAD_MONITOR_REDIS_HASH,ExceptionCollector.getThreadMonitorKeyByField(field));
            if ( !StringUtils.isEmpty(value) ) {
                long lastExecutorTime = Long.valueOf(value);
                result = System.currentTimeMillis()/1000 - lastExecutorTime > ExceptionCollector.timeInterval;
            }
