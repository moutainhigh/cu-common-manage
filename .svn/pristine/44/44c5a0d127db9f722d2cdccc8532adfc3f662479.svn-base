...
public class PushSecurityTool {
	private String password = "LLX239"; // PUSH认证密码,默认值
	private String aesKey = "LVVKVXWF56496929"; // aes秘钥
	private String aesIv = "MSFBUWFS86537785"; // aes初始化向量
	private String rzKey = "LLX239"; // 消息认证秘钥
	private String randVal = Tools.getRandomString(); // 随机数

	private int encryptAlgorithm = 1; // 0-无加密，1-AES加密
	private int hashAlgorithm = 1; // 0-无HASH，1-MD5，2-SHA-1
	private int compressionFormat = 1; // 0-无压缩，1-ZIP压缩
	private String pushVersion = "v1.0"; // PUSH接口版本
	
	private Logger logger = Logger.getLogger(PushSecurityTool.class);

	public PushSecurityTool() {
		try {
			Map<String, String> pro = PushConfigUtils.getProperties();
			initConfigs(pro);
		} catch (Exception e) {
	...
	private void initConfigs(Map<String, String> pro) {
		String tmp = (String) pro.get("password");
		this.password = tmp == null ? this.password : tmp;

		tmp = (String) pro.get("aes_key");
		this.aesKey = tmp == null ? this.aesKey : tmp;

		tmp = (String) pro.get("aes_iv");
		this.aesIv = tmp == null ? this.aesIv : tmp;

		tmp = (String) pro.get("rz_key");
		this.rzKey = tmp == null ? this.rzKey : tmp;

		tmp = pro.get("encrypt_algorithm");
		if (tmp != null && !tmp.isEmpty()) {
			int tmpi = Integer.parseInt(tmp);
			this.encryptAlgorithm = tmpi;
		}

		tmp = pro.get("hash_algorithm");
		if (tmp != null && !tmp.isEmpty()) {
			int tmpi = Integer.parseInt(tmp);
			this.hashAlgorithm = tmpi;
		}

		tmp = pro.get("compression_format");
		if (tmp != null && !tmp.isEmpty()) {
			int tmpi = Integer.parseInt(tmp);
			this.compressionFormat = tmpi;
		}

		tmp = (String) pro.get("version");
		this.pushVersion = tmp == null ? this.pushVersion : tmp;
	}
	...
	public PushEncryptResult encrypt(String dataStr) throws Exception {
		...
		if (this.compressionFormat == PushSecurityTool.COMPRESSION_FORMAT_ZIP) {
	...
	public static void main(String[] args) {
		try {
			...
			PushDecryptResult drs = tool.decrypt(rs.getRandVal(), rs.getPwdHash(), rs.getPush(), rs.getPushHash(), rs.getEncryptAlgorithm(), rs.getHashAlgorithm(), rs.getCompressionFormat(),
					rs.getPushVersion());
	...
	public class PushEncryptResult {
		...
		public PushEncryptResult() {
			...
			this.compressionFormat = PushSecurityTool.this.compressionFormat;
		...
		public String toString() {
			return "PushEncryptResult [randVal=" + randVal + ", pwdHash=" + pwdHash + ", push=" + push + ", pushHash=" + pushHash + ", encryptAlgorithm=" + encryptAlgorithm + ", hashAlgorithm="
					+ hashAlgorithm + ", commpresssionFormat=" + compressionFormat + ", pushVersion=" + pushVersion + "]";
		}
	...
	public static class Tools {
		...
		public static String getBase64(String source) {
			try {
				return new String(org.apache.commons.codec.binary.Base64.encodeBase64(source.getBytes("UTF-8")));
		...
		public static String encodeBase64(String param) {
			if (StringUtils.isNotBlank(param)) {
				try {
					param = org.apache.commons.codec.binary.Base64.encodeBase64String(param.getBytes("UTF-8"));
				} catch (UnsupportedEncodingException e) {
					param = org.apache.commons.codec.binary.Base64.encodeBase64String(param.getBytes());
				}
			}
			return param;
		}
		...
		public static String decodeBase64(String param) {
			...
			if (StringUtils.isNotBlank(param)) {
				try {
					data = new String(org.apache.commons.codec.binary.Base64.decodeBase64(param.getBytes("UTF-8")),"UTF-8");
				} catch (UnsupportedEncodingException e) {
					data = new String(org.apache.commons.codec.binary.Base64.decodeBase64(param));
				}
			}
			return data;
		}
